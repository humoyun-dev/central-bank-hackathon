...
import { updateAccountRequestSchema } from "@/features/accounts/schemas/update-account"
import { hasPermission } from "@/lib/permissions"
import { updateMockAccount } from "@/services/api/mock/store"
...
export async function PATCH(
  request: Request,
  context: { params: Promise<{ householdId: string; accountId: string }> },
) {
  ...
  if (!hasPermission(household.role, "manageSettings")) {
    return domainProblemJson({
      ...
      detail: "This membership cannot update household accounts.",
      status: 403,
    })
  }
  ...
  const parsedPayload = updateAccountRequestSchema.safeParse(rawPayload)
  ...
  try {
    const account = updateMockAccount({
      householdId,
      accountId,
      ...parsedPayload.data,
    })

    return Response.json(accountDtoSchema.parse(account), { status: 200 })
  } catch (error) {
    return domainProblemJson({
